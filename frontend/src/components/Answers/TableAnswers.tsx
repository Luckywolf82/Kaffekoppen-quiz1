...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/answers/answersSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureAnswersCols';
...
const TableSampleAnswers = ({
  ...
  useEffect(() => {
    if (answersNotify.showNotification) {
      notify(answersNotify.typeNotification, answersNotify.textNotification);
    }
  }, [answersNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/answers/answers-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {answers && Array.isArray(answers) && !showGrid && (
        <ListAnswers
          answers={answers}
          loading={loading}
...
export default TableSampleAnswers;
