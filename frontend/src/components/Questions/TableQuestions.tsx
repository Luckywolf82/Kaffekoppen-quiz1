...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/questions/questionsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureQuestionsCols';
...
const TableSampleQuestions = ({
  ...
  useEffect(() => {
    if (questionsNotify.showNotification) {
      notify(
        questionsNotify.typeNotification,
        questionsNotify.textNotification,
      );
    }
  }, [questionsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/questions/questions-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {questions && Array.isArray(questions) && !showGrid && (
        <ListQuestions
          questions={questions}
          loading={loading}
...
export default TableSampleQuestions;
