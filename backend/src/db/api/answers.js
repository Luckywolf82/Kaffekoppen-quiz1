...
module.exports = class AnswersDBApi {
  static async create(data, options) {
    ...
    await answers.setQuestion(data.question || null, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.question = await answers.getQuestion({
      transaction,
    });
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    if (filter) {
      ...
      if (filter.question) {
        var listItems = filter.question.split('|').map((item) => {
          return Utils.uuid(item);
        });

        where = {
          ...where,
          questionId: { [Op.or]: listItems },
        };
      }
