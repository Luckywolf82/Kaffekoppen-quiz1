...
module.exports = class OrganizationsDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.users_organization = await organizations.getUsers_organization({
      transaction,
    });

    output.answers_organization = await organizations.getAnswers_organization({
      transaction,
    });

    output.images_organization = await organizations.getImages_organization({
      transaction,
    });

    output.payments_organization = await organizations.getPayments_organization(
      {
        transaction,
      },
    );

    output.questions_organization =
      await organizations.getQuestions_organization({
        transaction,
      });

    output.quizzes_organization = await organizations.getQuizzes_organization({
      transaction,
    });
