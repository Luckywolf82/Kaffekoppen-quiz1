...
module.exports = class QuestionsDBApi {
  static async create(data, options) {
    ...
    await questions.setQuiz(data.quiz || null, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.answers_question = await questions.getAnswers_question({
      transaction,
    });

    output.quiz = await questions.getQuiz({
      transaction,
    });
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    if (filter) {
      ...
      if (filter.quiz) {
        var listItems = filter.quiz.split('|').map((item) => {
          return Utils.uuid(item);
        });

        where = {
          ...where,
          quizId: { [Op.or]: listItems },
        };
      }
