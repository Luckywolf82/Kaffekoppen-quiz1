...
module.exports = class UsersDBApi {
  static async create(data, globalAccess, options) {
    ...
    await users.setOrganization(
      (globalAccess ? data.data.organization : currentUser.organization.id) ||
        null,
      {
        transaction,
      },
  ...
  static async findBy(where, options) {
    ...
    output.images_uploaded_by = await users.getImages_uploaded_by({
      transaction,
    });

    output.payments_user = await users.getPayments_user({
      transaction,
    });
    ...
    output.organization = await users.getOrganization({
      transaction,
    });
