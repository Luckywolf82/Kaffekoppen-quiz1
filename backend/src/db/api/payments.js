...
module.exports = class PaymentsDBApi {
  static async create(data, options) {
    ...
    await payments.setUser(data.user || null, {
      transaction,
    });
  ...
  static async update(id, data, options) {
    ...
    await payments.setOrganization(
  ...
  static async findBy(where, options) {
    ...
    output.user = await payments.getUser({
      transaction,
    });

    output.organization = await payments.getOrganization({
      transaction,
    });
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    if (filter) {
      ...
      if (filter.gateway) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('payments', 'gateway', filter.gateway),
        };
      }
  ...
  static async findAllAutocomplete(query, limit, globalAccess, organizationId) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('payments', 'gateway', query),
        ],
      };
    }

    const records = await db.payments.findAll({
      attributes: ['id', 'gateway'],
      where,
      limit: limit ? Number(limit) : undefined,
      orderBy: [['gateway', 'ASC']],
    });
